...
<?php if (isset($_GET['deleted'])): ?><div class="srt-alert srt-alert-success"><i class="fa-solid fa-trash me-2"></i>Đã xoá liên hệ.</div><?php endif; ?>

<div class="srt-card">
    <div class="srt-card-header"><span><i class="fa-solid fa-list me-2"></i>Danh sách liên hệ</span></div>
    <div class="srt-card-body" style="padding:0;">
        <div style="overflow-x:auto;">
        <table class="srt-table">
            <thead><tr>
                <th style="width:40px;">#</th>
                <th>Người gửi</th><th>Email</th><th>Nội dung</th>
                <th>Trạng thái</th><th>Ngày gửi</th><th style="width:160px;">Thao tác</th>
            </tr></thead>
            <tbody>
            <?php
            $stt = ($page - 1) * 15 + 1;
            if ($contacts->num_rows === 0): ?>
                <tr><td colspan="7" class="text-center py-5" style="color:#aaa;">
            ...
            <?php else: while ($c = $contacts->fetch_assoc()): ?>
                <tr>
                    ...
                    <td><a href="mailto:<?= htmlspecialchars($c['email']) ?>" class="text-decoration-none" style="font-size:.85rem;color:var(--accent);"><?= htmlspecialchars($c['email']) ?></a></td>
                    <td style="max-width:260px;">
                        <span style="font-size:.83rem;color:#555;"><?= htmlspecialchars(mb_substr($c['message'], 0, 80)) ?><?= mb_strlen($c['message']) > 80 ? '…' : '' ?></span><br>
                    ...
                    <td><?php
                        $sm = ['new'=>['Mới','badge-new'],'read'=>['Đã đọc','badge-read'],'replied'=>['Đã phản hồi','badge-replied']];
                        $s  = $sm[$c['status']] ?? $sm['new'];
                        echo '<span class="'.$s[1].'">'.$s[0].'</span>';
                    ?></td>
                    <td style="font-size:.82rem;color:#888;white-space:nowrap;"><?= $c['created_at'] ? date('d/m/Y H:i', strtotime($c['created_at'])) : '—' ?></td>
                    <td>
                        <div class="d-flex gap-1 flex-wrap">
                            <div class="dropdown">
                                <button class="btn-srt-primary btn-srt-sm dropdown-toggle" data-bs-toggle="dropdown" style="cursor:pointer;"><i class="fa-solid fa-tag"></i></button>
                                <ul class="dropdown-menu shadow border-0" style="font-size:.82rem;min-width:150px;">
                                    ...
                                    <li>
                                        <form method="POST" action="<?= BASE_PATH ?>/admin/contacts/update/<?= $c['id'] ?>">
                                            ...
                                            <button type="submit" class="dropdown-item <?= $c['status']===$val?'fw-bold':'' ?>"><?= $c['status']===$val?'✓ ':'' ?><?= $label ?></button>
                                        </form>
                                    </li>
                                    <?php endforeach; ?>
                                </ul>
                            </div>
                            <form method="POST" action="<?= BASE_PATH ?>/admin/contacts/delete/<?= $c['id'] ?>" onsubmit="return confirm('Xác nhận xoá?')">
                                <?= csrf_field() ?>
        ...
        <div style="padding:16px 20px;"><div class="srt-pagination">
            <?php if ($page > 1): ?><a href="<?= BASE_PATH ?>/admin/contacts?page=<?= $page-1 ?>"><i class="fa-solid fa-chevron-left"></i></a><?php else: ?><span class="disabled"><i class="fa-solid fa-chevron-left"></i></span><?php endif; ?>
            <?php for ($i=1;$i<=$pages;$i++): ?>
                <?php if ($i==$page): ?><span class="active"><?= $i ?></span>
                <?php elseif (abs($i-$page)<=2||$i==1||$i==$pages): ?><a href="<?= BASE_PATH ?>/admin/contacts?page=<?= $i ?>"><?= $i ?></a>
                <?php elseif (abs($i-$page)==3): ?><span class="disabled">…</span><?php endif; ?>
            <?php endfor; ?>
            <?php if ($page<$pages): ?><a href="<?= BASE_PATH ?>/admin/contacts?page=<?= $page+1 ?>"><i class="fa-solid fa-chevron-right"></i></a><?php else: ?><span class="disabled"><i class="fa-solid fa-chevron-right"></i></span><?php endif; ?>
        </div></div>
        <?php endif; ?>
    </div>
</div>
...
<div class="modal fade" id="msgModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4">
            ...
            <div class="modal-body pt-2" id="msgModalBody" style="white-space:pre-wrap;font-size:.9rem;color:#444;line-height:1.7;max-height:400px;overflow-y:auto;"></div>
        </div>
    </div>
</div>
<script>
